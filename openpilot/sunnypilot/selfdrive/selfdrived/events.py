...
EVENTS_SP: dict[int, dict[str, Alert | AlertCallbackType]] = {
  ...
  EventNameSP.stockLkasOff: {
    # Mazda: invalidLkasSetting is swapped for this when MADS is on (CarSpecificEventsSP). No
    # alert of its own: the button press on the same frame already speaks (lkasDisable or
    # manualSteeringRequired), and on a TJA car the cluster names the master the driver
    # switched; the no-entry is for later enable attempts with LKA still off.
