  ...
  def update(self, CS: structs.CarState, events: Events, CS_SP, mads_enabled_toggle: bool = False):
    ...
    if self.CP.brand == 'chrysler':
      ...
      if mads_enabled_toggle and (CS.invalidLkasSetting or CS.lkaButtonOff) and self.CP_SP.flags & MazdaFlagsSP.LKA_BUTTON:
        # MADS enabled: the dash LKA button is the lateral toggle, so LKA off at either
        # switch is a driver choice: the selfdrive machine still engages on the stock
        # cruise, the MADS machine alone refuses lateral. Otherwise the no-entry stands.
