...
# The gauge shows cylinder mode: two arcs grow clockwise as the PCM ramps
...
# fades out and the gauge hides. Cars without deactivation hardware never
# leave normal, so the gauge stays hidden without a setting.
#
...
class CylinderDeactivationRenderer:
  ...
  def render(self, rect: rl.Rectangle, sm) -> None:
    ...
    progress = float(cd.entryProgress)

    if state == STATE.normal:
      # fade the previous state out, then hide
